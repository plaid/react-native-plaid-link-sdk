...
export default function App() {
  ...
  if (screen === 'plaidLinkSession') return <PlaidLinkSessionScreen onBack={() => setScreen('list')} />;
  if (screen === 'plaidLinkHeadlessSession') return <PlaidLinkHeadlessSessionScreen onBack={() => setScreen('list')} />;
  if (screen === 'plaidLayerSession') return <PlaidLayerSessionScreen onBack={() => setScreen('list')} />;
  if (screen === 'plaidEmbeddedSearch') return <PlaidEmbeddedSearchScreen onBack={() => setScreen('list')} />;
  if (screen === 'syncFinanceKit') return <SyncFinanceKitScreen onBack={() => setScreen('list')} />;

  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.container}>
        <Text style={styles.header}>LinkKit Examples</Text>
        <Text style={styles.sdkVersion}>SDK: {ReactNativePlaidLinkSdk.sdkVersion}</Text>
        {SCREENS.map((s) => (
          <TouchableOpacity key={s.key} style={styles.row} onPress={() => setScreen(s.key)}>
