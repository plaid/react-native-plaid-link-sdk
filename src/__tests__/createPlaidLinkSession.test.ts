...
describe('createPlaidLinkSession', () => {
  ...
  it('creates session with valid token and callbacks', async () => {
    ...
    expect(NativePlaidModule.createPlaidLinkSession).toHaveBeenCalledWith('link-sandbox-token-123');
    ...
    expect(NativePlaidModule.addListener).toHaveBeenCalledWith('onSuccess', expect.any(Function));
    expect(NativePlaidModule.addListener).toHaveBeenCalledWith('onExit', expect.any(Function));
    expect(NativePlaidModule.addListener).toHaveBeenCalledWith('onEvent', expect.any(Function));
  });
  ...
  it('cleans up existing listeners before creating new session', async () => {
    ...
    const firstListenerCalls = (NativePlaidModule.addListener as jest.Mock).mock.calls.length;
    
    jest.clearAllMocks();
    
    await createPlaidLinkSession(config2);
    const secondListenerCalls = (NativePlaidModule.addListener as jest.Mock).mock.calls.length;
  ...
  it('handles errors from native module', async () => {
    ...
    (NativePlaidModule.createPlaidLinkSession as jest.Mock).mockRejectedValueOnce(mockError);
    ...
    await expect(createPlaidLinkSession(config)).rejects.toThrow('Native session creation failed');
    expect(console.error).toHaveBeenCalledWith('[PlaidLink] createPlaidLinkSession failed:', mockError);
  });
